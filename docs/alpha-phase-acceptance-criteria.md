...
明示的に依存）。**#17・#34とも実装済みで、この完了条件は満たされている。**
...
- **#34（ドラッグ移動）分も実装済み**: `scenarios/phaseB-drag.mjs` /
  `orchestrate-phase-b-drag.mjs`。`TableNode.tsx` が公開する
  `data-testid="table-node"` の `data-x`/`data-y` をアサーション対象にし、
  「ドラッグでテーブルが移動しスナップバックしない」「ドラッグ中はキャンバスの
  ズームが変わらず、非ドラッグテーブルが画面上で静止している（＝`Canvas.tsx`が
  左/上ドラッグ時の`viewBox`原点シフトぶんパン変換を`useLayoutEffect`で即時
  補正する）」「ドラッグ後に別文を実行しても位置が保持され、
  かつ新規テーブルがドラッグ済みテーブルと重ならない位置に生成される」
  「テーブルを原点を跨いで左上へドラッグしてもクランプされず、
  `data-world-min-x`/`-min-y`（`Canvas.tsx`が公開）がそれに応じて負に
  なりキャンバスが左・上方向へ伸び、かつ左上端テーブル自身はカーソルに
  追従する（ピン留めされない）」を検証する。
  実装時、ヘッダーの`onPointerDown`での`event.stopPropagation()`では
  react-zoom-pan-pinchの`window`直接`mousedown`リスナー（Reactのイベント委譲と
  無関係）を止められないことをこの受け入れチェックで実際に検出し、
  `panning.excluded`（CSSクラスベース）方式に修正した（詳細はCLAUDE.md参照）。
  `#34`本体の未決事項（ALTER ADD/DROP COLUMNによる手動配置済みテーブルの
  リサイズ時に再配置が必要か）はIssue本文の通り実装後の使用感確認まで意図的に
  据え置いており、`phaseB-drag.mjs`はこのケースを検証しない。
