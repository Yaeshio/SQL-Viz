...
C-1（SMOKE-11〜13）は `parseError` に `Unsupported statement type: ...` が
含まれることを検証する、通常のエラー系テストと同じ書き方でよい。

C-2（SMOKE-14〜16）は、テストコード中に以下の趣旨のコメントを付す。

```ts
// SMOKE-14: JOIN は現状エラーにならず、FROM 句の先頭テーブルのみが処理される。
// これは意図された仕様ではなく既知のバグである（smoke-test-spec.md 4節を参照）。
// 修正時（別 issue で対応予定）には、このテストを「Unsupported clause エラーになる」
// ことを検証する正常系テストへ書き換えること。
it('SMOKE-14: JOINは無エラーで先頭テーブルのみ処理される（既知のバグ）', () => {
  // ...
});
```

test-design.md の `LAYOUT-Y-01`（layoutTables の現状挙動を固定する特性テスト）とは
性質が異なる点に注意する。`LAYOUT-Y-01` は「仕様として妥当な現状挙動の固定」で
あるのに対し、`SMOKE-14`〜`16` は「バグである現状挙動の一時的な固定」である。
コメント文言でこの違いを明確に区別し、両者を安易に同一パターンとして扱わない。
...
- C-2 区分（JOIN・複合WHERE・GROUP BY等の黙殺）の修正タイミング・issue番号は未定。
  本書では「スモークテスト整備完了後に別 issue として起票・対応する」という順序のみを
  確定事項として記載し、issue 起票自体は本タスクのスコープ外とする。
