...
| ケースID | シナリオ | 検証内容 |
|---|---|---|
| SMOKE-17 | `CREATE TABLE` のみを含むSQLを実行した後、エディタの内容を別の `INSERT` のみのSQLに置き換えて再度実行する（＝2回の独立した `runPipeline` 呼び出しで、2回目の `initialState` に1回目の最終 `DBState` を渡す） | 2回目の実行で `table_appear` が再発火せず、既存テーブルに行が追加されること（イベントが `row_add` のみになること）。最終的な行数・`state.order` が正しいこと |
