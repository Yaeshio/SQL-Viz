...
describe('イベント生成層（SQL → parser → reducer → diffStates の統合テスト）', () => {
  it('EVENT-01: CREATE TABLE のみでは table_appear のみ発生する（row_add は発生しない）', () => {
    const results = runSqlStatements(['CREATE TABLE users (id INT, name VARCHAR(50))']);
    expect(results).toHaveLength(1);
    expect(results[0].events).toEqual([{ kind: 'table_appear', table: 'users' }]);
  });

  it('EVENT-02: CREATE TABLE 後に複数行の INSERT を行うと、各文の結果に対応するイベントが出る', () => {
    const results = runSqlStatements([
  ...
  it('EVENT-03: 既存データに対する INSERT 1件では追加した1行分の row_add のみ発生する（table_appear は発生しない）', () => {
    const results = runSqlStatements([
      'CREATE TABLE users (id INT, name VARCHAR(50))',
      "INSERT INTO users (id, name) VALUES (1, 'Alice'), (2, 'Bob')",
  ...
  it('EVENT-04: INSERT 後の SELECT ... WHERE ... では条件に合わない行の row_filter と select_highlight が発生する', () => {
    const results = runSqlStatements([
      'CREATE TABLE users (id INT, name VARCHAR(50))',
  ...
  it('EVENT-05: フィルタされた状態から WHERE なしで再度 SELECT すると row_unfilter と select_highlight が発生する', () => {
    const results = runSqlStatements([
      'CREATE TABLE users (id INT, name VARCHAR(50))',
      "INSERT INTO users (id, name) VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Carol')",
      'SELECT name FROM users WHERE id > 1',
  ...
  it('EVENT-06: CREATE → INSERT → SELECT の3文連続実行で、文ごとに独立したイベント列が累積した状態の上に生成される', () => {
    const results = runSqlStatements([
      'CREATE TABLE users (id INT, name VARCHAR(50))',
  ...
  it('EVENT-07: 途中の文でエラーとなる場合（存在しないテーブルへの INSERT）はそれ以降の文が処理されない', () => {
    expect(() =>
      runSqlStatements([
