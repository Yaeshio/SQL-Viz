...
- 集計（`GROUP BY`）・整列（`ORDER BY`）・`LIMIT`
- サブクエリ・`UNION`・トランザクション

これらは `docs/Sql animation tool spec .md` の対応フェーズ表（Phase 2〜4）に沿って
今後拡張していく想定です。
...
1文ずつ順番に通過します（`src/App.tsx` の `run()` が駆動）。
...
[4] diff.ts     — diffStates(old, next) が新旧の DBState を比較し、
   ...
   ↓
[5] App.tsx     — playEvents() がイベントを順に処理し、React state を
                  更新しながら Canvas.tsx（framer-motion）でアニメーション再生
```
...
```
src/
  App.tsx      # 画面全体・実行パイプラインの駆動
  Canvas.tsx   # SVG + framer-motion によるテーブル/行の描画
  parser.ts    # SQL → AST → Parsed* 型への変換
  reducer.ts   # DBState への純粋な適用ロジック
  layout.ts    # テーブルのグリッドレイアウト計算
